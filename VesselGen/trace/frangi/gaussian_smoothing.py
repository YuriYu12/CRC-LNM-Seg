...
class GaussianSmoothing(nn.Module):
    ...
    def __init__(self, channels, kernel_size, sigma, dim=2, order=0, device=None):
        ...
        if order == 'xx':
            ...
            kernel[0, 0] = ((kernel[0, 0] * ((meshgrids[0].transpose(0, 1) - self.padd) ** 2 - self.std ** 2) / self.std ** 4))
        elif order == 'zz':
            kernel[0, 0] = ((kernel[0, 0] * ((meshgrids[0].transpose(0, 2) - self.padd) ** 2 - self.std ** 2) / self.std ** 4))
        elif order == 'xy':
            kernel[0, 0] = kernel[0, 0] * (meshgrids[0] - self.padd) * (meshgrids[0].transpose(0, 1) - self.padd) / self.std ** 4
        elif order == 'yz':
            kernel[0, 0] = kernel[0, 0] * (meshgrids[0].transpose(0, 1) - self.padd) * (meshgrids[0].transpose(0, 2) - self.padd) / self.std ** 4
        elif order == 'xz':
            kernel[0, 0] = kernel[0, 0] * (meshgrids[0] - self.padd) * (meshgrids[0].transpose(0, 2) - self.padd) / self.std ** 4
