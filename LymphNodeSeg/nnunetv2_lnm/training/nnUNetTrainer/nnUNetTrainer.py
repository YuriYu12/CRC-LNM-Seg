...
from nnunetv2_lnm.configuration import ANISO_THRESHOLD, default_num_processes
from nnunetv2_lnm.evaluation.evaluate_predictions import compute_metrics_on_folder
from nnunetv2_lnm.inference.export_prediction import export_prediction_from_logits, resample_and_save
from nnunetv2_lnm.inference.predict_from_raw_data import nnUNetPredictor
from nnunetv2_lnm.inference.sliding_window_prediction import compute_gaussian
from nnunetv2_lnm.paths import nnUNet_preprocessed, nnUNet_results
from nnunetv2_lnm.training.data_augmentation.compute_initial_patch_size import get_patch_size
from nnunetv2_lnm.training.data_augmentation.custom_transforms.cascade_transforms import MoveSegAsOneHotToData, \
    ApplyRandomBinaryOperatorTransform, RemoveRandomConnectedComponentFromOneHotEncodingTransform
from nnunetv2_lnm.training.data_augmentation.custom_transforms.deep_supervision_donwsampling import \
    DownsampleSegForDSTransform2
from nnunetv2_lnm.training.data_augmentation.custom_transforms.limited_length_multithreaded_augmenter import \
    LimitedLenWrapper
from nnunetv2_lnm.training.data_augmentation.custom_transforms.masking import MaskTransform
from nnunetv2_lnm.training.data_augmentation.custom_transforms.region_based_training import \
    ConvertSegmentationToRegionsTransform
from nnunetv2_lnm.training.data_augmentation.custom_transforms.transforms_for_dummy_2d import Convert2DTo3DTransform, \
    Convert3DTo2DTransform
from nnunetv2_lnm.training.dataloading.data_loader_2d import nnUNetDataLoader2D
from nnunetv2_lnm.training.dataloading.data_loader_3d import nnUNetDataLoader3D
from nnunetv2_lnm.training.dataloading.nnunet_dataset import nnUNetDataset
from nnunetv2_lnm.training.dataloading.utils import get_case_identifiers, unpack_dataset
from nnunetv2_lnm.training.logging.nnunet_logger import nnUNetLogger
from nnunetv2_lnm.training.loss.compound_losses import DC_and_CE_loss, DC_and_BCE_loss
from nnunetv2_lnm.training.loss.deep_supervision import DeepSupervisionWrapper
from nnunetv2_lnm.training.loss.dice import get_tp_fp_fn_tn, MemoryEfficientSoftDiceLoss
from nnunetv2_lnm.training.lr_scheduler.polylr import PolyLRScheduler
from nnunetv2_lnm.utilities.collate_outputs import collate_outputs
from nnunetv2_lnm.utilities.default_n_proc_DA import get_allowed_n_proc_DA
from nnunetv2_lnm.utilities.file_path_utilities import check_workers_alive_and_busy
from nnunetv2_lnm.utilities.get_network_from_plans import get_network_from_plans
from nnunetv2_lnm.utilities.helpers import empty_cache, dummy_context
from nnunetv2_lnm.utilities.label_handling.label_handling import convert_labelmap_to_one_hot, determine_num_input_channels
from nnunetv2_lnm.utilities.plans_handling.plans_handler import PlansManager, ConfigurationManager
...
class nnUNetTrainer(object):
            ...
            for k in self.__dir__():
                ...
                if k in ['dataloader_train', 'dataloader_val']:
                    ...
                    if hasattr(getattr(self, k), 'num_processes'):
                        dct[k +
                            '.num_processes'] = str(getattr(self, k).num_processes)
    ...
    def _set_batch_size_and_oversample(self):
        if not self.is_ddp:
            ...
            for rank in range(world_size):
                ...
                sample_id_low = 0 if len(
                    batch_sizes) == 0 else np.sum(batch_sizes[:-1])
                ...
                if sample_id_high / global_batch_size < (1 - self.oversample_foreground_percent):
                    ...
                    percent_covered_by_this_rank = sample_id_high / \
                        global_batch_size - sample_id_low / global_batch_size
    ...
    def do_split(self):
        ...
        if self.fold == "all":
            ...
            case_identifiers = get_case_identifiers(
    ...
    def get_dataloaders(self):
        ...
        if allowed_num_processes == 0:
            ...
            mt_gen_val = LimitedLenWrapper(self.num_val_iterations_per_epoch, data_loader=dl_val,
                                           transform=val_transforms, num_processes=max(
                                               1, allowed_num_processes // 2),
    ...
    def get_training_transforms(patch_size: Union[np.ndarray, Tuple[int]],
                                ...
                                foreground_labels: Union[Tuple[int, ...],
                                                         List[int]] = None,
                                regions: List[Union[List[int],
                                                    Tuple[int, ...], int]] = None,
    ...
    def on_train_start(self):
        ...
        save_json(self.plans_manager.plans, join(
            self.output_folder_base, 'plans.json'), sort_keys=False)
        save_json(self.dataset_json, join(
            self.output_folder_base, 'dataset.json'), sort_keys=False)
    ...
    def on_epoch_end(self):
        ...
        if (current_epoch + 1) % self.save_every == 0 and current_epoch != (self.num_epochs - 1):
            ...
            self.save_checkpoint(
                join(self.output_folder, 'checkpoint_%s.pth' % str(current_epoch + 1)))
    ...
    def perform_actual_validation(self, save_probabilities: bool = False, use_checkpoint: str = 'final'):
        ...
        with multiprocessing.get_context("spawn").Pool(default_num_processes) as segmentation_export_pool:
            ...
            validation_output_folder = join(self.output_folder, 'validation_%s' % use_checkpoint)
            ...
            if next_stages is not None:
                _ = [maybe_mkdir_p(
                    join(self.output_folder_base, 'predicted_next_stage', n)) for n in next_stages]
            ...
            for k in dataset_val.keys():
                ...
                if next_stages is not None:
                    for n in next_stages:
                        next_stage_config_manager = self.plans_manager.get_configuration(
                            n)
        ...
        if self.local_rank == 0:
            metrics = compute_metrics_on_folder(join(self.preprocessed_dataset_folder_base, 'gt_segmentations'),
                                                ...
                                                join(
                                                    validation_output_folder, 'summary.json'),
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            with torch.no_grad():
                ...
                for batch_id in range(self.num_val_iterations_per_epoch):
                    val_outputs.append(self.validation_step(
                        next(self.dataloader_val)))
