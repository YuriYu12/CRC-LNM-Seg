...
def resample_data_or_seg_to_shape(data: Union[torch.Tensor, np.ndarray],
                                  ...
                                  force_separate_z: Union[bool, None] = False,
                                  separate_z_anisotropy_threshold: float = ANISO_THRESHOLD,
                                  dataset_json: Union[dict, str] = None,
                                  ):
    """
    ...
    data_reshaped = resample_data_or_seg(data, new_shape, is_seg, axis, order, do_separate_z, order_z=order_z, dataset_json=dataset_json)
    return data_reshaped
...
def resample_data_or_seg(data: np.ndarray, new_shape: Union[Tuple[float, ...], List[float], np.ndarray],
    ...
    if np.any(shape != new_shape):
        ...
        if do_separate_z:
            ...
            if dataset_json is not None and len(dataset_json['channel_names']) > 1 and dataset_json['channel_names']['1'] == 'vessel':
                reshaped = []
                for c in range(data.shape[0]):
                    if c == 1:
                        reshaped.append(resize_segmentation(data[c], new_shape, order)[None])
                    else:
