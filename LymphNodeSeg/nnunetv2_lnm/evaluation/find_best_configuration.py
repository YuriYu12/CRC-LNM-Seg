...
from nnunetv2_lnm.ensembling.ensemble import ensemble_crossvalidations
from nnunetv2_lnm.evaluation.accumulate_cv_results import accumulate_cv_results
from nnunetv2_lnm.evaluation.evaluate_predictions import compute_metrics_on_folder, load_summary_json
from nnunetv2_lnm.paths import nnUNet_preprocessed, nnUNet_raw, nnUNet_results
from nnunetv2_lnm.postprocessing.remove_connected_components import determine_postprocessing
from nnunetv2_lnm.utilities.file_path_utilities import maybe_convert_to_dataset_name, get_output_folder, \
...
def accumulate_crossval_results_entry_point():
    ...
    parser.add_argument('-chk', type=str, required=False, default='final')
    args = parser.parse_args()
    trained_model_folder = get_output_folder(args.dataset_name_or_id, args.tr, args.p, args.c)

    if args.o is None:
        merged_output_folder = join(trained_model_folder, f'crossval_results_folds_{folds_tuple_to_string(args.f)}_{args.chk}')
    else:
        merged_output_folder = args.o

    accumulate_cv_results(trained_model_folder, merged_output_folder, args.f, folder=args.chk)
