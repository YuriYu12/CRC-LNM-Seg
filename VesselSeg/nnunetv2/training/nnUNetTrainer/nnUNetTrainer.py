...
# from nnunetv2.training.loss.compound_losses import DC_and_CE_loss, DC_and_BCE_loss, DC_and_clDC_loss
import nnunetv2.training.loss.compound_losses as losses
from nnunetv2.training.loss.deep_supervision import DeepSupervisionWrapper
from nnunetv2.training.loss.dice import get_tp_fp_fn_tn, MemoryEfficientSoftDiceLoss, SoftDiceLoss
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 ...
                 start_lr: float = 0.01, max_ep: int = 1000, desc: str=None, weight_decay=3e-5,
        ...
        self.output_folder_base = join(nnUNet_results,
                                        ...
                                        "__".join([self.__class__.__name__, self.plans_manager.plans_name, configuration, desc])) \
            if nnUNet_results is not None else None
        self.output_folder = join(self.output_folder_base, f'fold_{fold}')
        # maybe_mkdir_p(join(self.output_folder, "nnunetv2_training_code"))  # sliences line 172
        # shutil.copytree(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), join(self.output_folder, "nnunetv2_training_code"), 
        ...
        self.oversample_foreground_percent = foreground_oversampling
        self.num_iterations_per_epoch = 250
        self.num_val_iterations_per_epoch = 50
        self.num_epochs = max_ep
        self.current_epoch = 0

        ### Dealing with labels/regions
        self.label_manager = self.plans_manager.get_label_manager(dataset_json)
        # init the perceived prior carry vector for all labels
        self.perceived_prior_carry = np.zeros((len(self.label_manager.all_labels),))
        self.actual_prior_carry = np.zeros((len(self.label_manager.all_labels),))
    ...
    def _build_loss(self):
        print(f"loss type {self.configuration_manager.loss_name}")
        if self.configuration_manager.loss_name == "ce_and_dice":
            if self.label_manager.has_regions:
                loss = losses.DC_and_BCE_loss({},
                                    {'batch_dice': self.configuration_manager.batch_dice,
                                        'do_bg': True, 'smooth': 1e-5, 'ddp': self.is_ddp},
                                    use_ignore_label=self.label_manager.ignore_label is not None,
                                    dice_class=MemoryEfficientSoftDiceLoss)
            else:
                loss = losses.DC_and_CE_loss({'batch_dice': self.configuration_manager.batch_dice,
                                            'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp}, {}, weight_ce=1, weight_dice=1,
                                            ignore_label=self.label_manager.ignore_label, dice_class=SoftDiceLoss)
        elif self.configuration_manager.loss_name == "cldice_and_dice":
            loss = losses.DC_and_clDC_loss(dict(batch_dice=self.configuration_manager.batch_dice, smooth=1e-5, do_bg=False, ddp=self.is_ddp),
                                            dict(batch_cldice=self.configuration_manager.batch_dice, iterations=10, do_bg=False, ddp=self.is_ddp),
                                            alpha=0.5, ignore_label=self.label_manager.ignore_label, dice_class=MemoryEfficientSoftDiceLoss)
        else:
            loss_fn = getattr(losses, self.configuration_manager.loss_name)
            if loss_fn is None:
                raise NotImplementedError(f"expected loss type to be in [**compound_losses], got unexpected loss {self.plans_manager.loss_name}")
            else:
                loss = loss_fn(ignore_label=self.label_manager.ignore_label,
                               soft_dice_kwargs=dict(batch_dice=self.configuration_manager.batch_dice, do_bg=True, smooth=1e-5, ddp=self.is_ddp))
    ...
    def on_train_epoch_end(self, train_outputs: List[dict]):
        ...
        if self.is_ddp:
            ...
            priors_tr = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(priors_tr, outputs['loss'])
            priors_here = np.vstack(priors_tr).mean(0)
        else:
            loss_here = np.mean(outputs['loss'])
            priors_here = np.mean(outputs["perceived_prior"], 0)
        
        self.logger.log('train_losses', loss_here, self.current_epoch)
        self.logger.log("train_perceived_prior", priors_here, self.current_epoch)
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        data = data.to(self.device, non_blocking=True)
        count_fn = lambda volume: torch.bincount(volume.flatten().to(torch.long), minlength=len(self.label_manager.all_labels)).to(torch.float32) / torch.numel(volume)
        if isinstance(target, list):
            target = [i.to(self.device, non_blocking=True) for i in target]
            self.actual_prior_carry = self.actual_prior_carry * 0.5 + np.mean([count_fn(t).data.cpu().numpy() for t in target], 0) * 0.5
        else:
            target = target.to(self.device, non_blocking=True)
            self.actual_prior_carry = self.actual_prior_carry * 0.5 + count_fn(target).data.cpu().numpy() * 0.5
        ...
        return {'loss': l.detach().cpu().numpy(), 'tp_hard': tp_hard, 'fp_hard': fp_hard, 'fn_hard': fn_hard, "actual_prior": self.actual_prior_carry}
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        actual_prior = np.mean(outputs_collated["actual_prior"], 0)
        ...
        if self.is_ddp:
            ...
            actual_priors = [None for _ in range(world_size)]
            dist.all_gather_object(losses_val, actual_prior)
            actual_prior = np.vstack([i[None] for i in actual_priors]).mean(0)
        else:
        ...
        self.logger.log("val_perceived_prior", actual_prior, self.current_epoch)
    ...
    def on_epoch_end(self):
        ...
        self.print_to_log_file("perceived_priors", np.round(self.logger.my_fantastic_logging['train_perceived_prior'][-1], decimals=8))
        self.print_to_log_file("actual_priors", np.round(self.logger.my_fantastic_logging['val_perceived_prior'][-1], decimals=8))
