...
def max_probabilities(list_of_files: List[str]) -> np.ndarray:
    assert len(list_of_files), 'At least one file must be given in list_of_files'
    avg = None
    for f in list_of_files:
        if avg is None:
            ...
            prob = np.load(f)["probabilities"]
            avg[1:] = np.where(avg[1:] > prob[1:], avg[1:], prob[1:])
    avg[0] = np.clip(1 - np.sum(avg[1:], axis=0), 0, 1)
    return avg
...
def merge_files(list_of_files,
    ...
    if avg_or_max == "avg": probabilities = average_probabilities(list_of_files)
    else: probabilities = max_probabilities(list_of_files)
    ...
    print(f"saved ensembling result to {output_filename_truncated}")
...
def ensemble_folders(list_of_input_folders: List[str],
    ...
    print(f"There are {len(s)} of individual predictions from {len(list_of_input_folders)} separate predict loops for me to ensemble")
...
def entry_point_ensemble_folders():
    ...
    parser.add_argument('--style', type=str, required=False, default="avg", help="use average or max prediction for foreground classes")

    args = parser.parse_args()
    ensemble_folders(args.i, args.o, args.save_npz, args.np, args.style)
