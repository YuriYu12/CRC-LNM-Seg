...
def convert_trainer_plans_config_to_identifier(trainer_name, plans_identifier, configuration, desc):
    p = f'{trainer_name}__{plans_identifier}__{configuration}__{desc}' if desc is not None else \
...
def get_output_folder(dataset_name_or_id: Union[str, int], trainer_name: str = 'nnUNetTrainer',
                      ...
                      fold: Union[str, int] = None, desc=None) -> str:
    tmp = join(nnUNet_results, maybe_convert_to_dataset_name(dataset_name_or_id),
               convert_trainer_plans_config_to_identifier(trainer_name, plans_identifier, configuration, desc))
    if fold is not None:
