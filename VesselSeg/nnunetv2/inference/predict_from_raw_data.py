...
def load_what_we_need(model_training_output_dir, use_folds, checkpoint_name, folder_name=None):
    ...
    for i, f in enumerate(use_folds):
        ...
        if not folder_name:
            select_ckpt = [foldername for foldername in os.listdir(join(model_training_output_dir)) if foldername.startswith(f"fold_{f}")]
            select_ckpt.sort(key=lambda tk: int(tk.split("_")[-1]))  # fold_{fold}_{time}_{index}
        else:
            select_ckpt = [""]
        checkpoint = torch.load(join(model_training_output_dir, select_ckpt[-1], checkpoint_name),
...
def predict_from_raw_data(list_of_lists_or_source_folder: Union[str, List[List[str]]],
    ...
    if device.type == 'cuda':
        device = torch.device(type='cuda', index=device.index)  # set the desired GPU with CUDA_VISIBLE_DEVICES!
    ...
    plans_manager, dataset_json, network, trainer_name = \
        load_what_we_need(model_training_output_dir, use_folds, checkpoint_name, fname)
    ...
    print(f'There are {sum([len(l) for l in list_of_lists_or_source_folder])} cases in the source folder')
            ...
            for preprocessed in mta:
                ...
                if perform_everything_on_gpu:
                    try:
                        ...
                        print('Error:')
                        # traceback.print_exc()
                        # the actual message is often too long that i want to omit it
                        print("(pretends to be the error msg, better start worry if there is really an error msg)")
                        prediction = None
...
def predict_entry_point():
    ...
    model_folder = get_output_folder(args.d, args.tr, args.p, args.c, args.desc)
